...
//
export const getOpenAPIObjectConfig = ({
  apiVersion,
}: {
  apiVersion: string;
}): Parameters<AppServer["getOpenAPIDocument"]>[0] => {
  return {
    info: {
      title,
      version: apiVersion,
...
const setupAPI = (
  server: AppServer,
  docSpecs: Parameters<typeof getOpenAPIObjectConfig>[0],
) => {
  server.use(
    `${routes.api.root}/*`,
    //
  ...
  server.doc(routes.swagger.doc, getOpenAPIObjectConfig(docSpecs));
};
